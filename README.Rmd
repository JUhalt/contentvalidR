...
**contentvalidR** provides tools for **substantive/content validity pretesting**:
- Sort-based indices from Anderson & Gerbing (1991): **Psa** and **Csv** (+ binomial test).
- Rating-based **ANOVA** approach from Hinkin & Tracey (1999) with Tukey post-hoc.
- Classic content validity indices: **CVR**, **IOC**, **Aiken’s V**, **CVI**.
- Diagnostics (signal detection, reproducibility) and small power simulators.
...
Create a tiny item-sort dataset: 4 items × 12 raters, with known targets.
```{r}
toy_sort <- data.frame(
...
Raters score each item on **every construct** (Likert-style). We test that the item’s mean on its **target** exceeds all others (ANOVA + Tukey).
```{r}
...
- `posthoc_pass = TRUE` means Tukey confirmed the **target mean > all others** at alpha = .05.

**3) Classic indices**

**Aiken’s V** (ordinal ratings on [1,5] scale), **CVR**, **CVI**, and **IOC**.
```{r}
...
Compare Csv-based decisions to a known “truth” (e.g., later CFA retention), and estimate reproducibility across two pretests.
```{r}
...
**Guidance**
- When to use what?
Use Psa/Csv for quick sort-based pretests; **ANOVA** for rating studies needing inferential evidence; triangulate with **Aiken’s V**, **CVR**, **CVI**, **IOC** for expert-panel contexts.
- Keep pretest **N (raters)** realistic (20–40 can go a long way).
- Be explicit about **target constructs** and instructions to raters.

**Citing the methods**
- Anderson, J. C., & Gerbing, D. W. (1991). Predicting the performance of measures in a confirmatory factor analysis with a pretest assessment of their substantive validities.

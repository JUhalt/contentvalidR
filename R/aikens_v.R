...
#'
#' @description
#' Computes Aiken's V per item from judges' ordinal ratings on a bounded scale defined by \code{lo} and \code{hi}.
...
#' @param lo,hi Numeric scale bounds (default 1 and 5).
...
aikens_v <- function(ratings, lo = 1, hi = 5,
  ...
  if (!all(R >= lo & R <= hi, na.rm = TRUE)) stop("ratings outside specified bounds")
  m <- nrow(R)
  k <- hi - lo + 1

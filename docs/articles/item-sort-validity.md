...
`  item ``=`` `[`rep`](https://rdrr.io/r/base/rep.html)`(`[`c`](https://rdrr.io/r/base/c.html)`(``"A1"``, ``"A2"``, ``"A3"``, ``"B1"``, ``"B2"``, ``"B3"``)``, each ``=`` ``20``)``,`\
`  rater ``=`` `[`rep`](https://rdrr.io/r/base/rep.html)`(``1``:``20``, ``6``)``,`\
`  target_construct ``=`` `[`rep`](https://rdrr.io/r/base/rep.html)`(`[`c`](https://rdrr.io/r/base/c.html)`(``"A"``, ``"A"``, ``"A"``, ``"B"``, ``"B"``, ``"B"``)``, each ``=`` ``20``)``,`\
...
`    `[`rep`](https://rdrr.io/r/base/rep.html)`(``"A"``, ``13``)``, `[`rep`](https://rdrr.io/r/base/rep.html)`(``"B"``, ``7``)``,`\
`    `[`rep`](https://rdrr.io/r/base/rep.html)`(``"B"``, ``18``)``, `[`rep`](https://rdrr.io/r/base/rep.html)`(``"A"``, ``2``)``,`\
`    `[`rep`](https://rdrr.io/r/base/rep.html)`(``"B"``, ``17``)``, `[`rep`](https://rdrr.io/r/base/rep.html)`(``"A"``, ``3``)``,`\
`    `[`rep`](https://rdrr.io/r/base/rep.html)`(``"B"``, ``14``)``, `[`rep`](https://rdrr.io/r/base/rep.html)`(``"A"``, ``6``)`\
`  ``)`\
`)`\
\
...
[`csv_binom_test`](https://juhalt.github.io/contentvalidR/reference/csv_binom_test.md)`(``n_c ``=`` ``14``, N ``=`` ``20``)`\
`#> $p.value`\
`#> [1] 0.05765915`\
`#> `\
`#> $estimate`\
...
\
[`colquitt_benchmarks`](https://juhalt.github.io/contentvalidR/reference/colquitt_benchmarks.md)`(``"psa"``)`\
...
`#> 4  20th-39th    0.39`\
`#> 5   0th-19th    -Inf`\
[`colquitt_benchmarks`](https://juhalt.github.io/contentvalidR/reference/colquitt_benchmarks.md)`(``"csv"``)`\
`#>   statistic benchmark_set                  benchmark_label interpretation`\
...
\
`fit_normed`` ``<-`` `[`sort_validity`](https://juhalt.github.io/contentvalidR/reference/sort_validity.md)`(`\
`  ``sort_dat``,`\
`  orbiting_r ``=`` `[`c`](https://rdrr.io/r/base/c.html)`(``A ``=`` ``.42``, B ``=`` ``.28``)`\
`)`\
...
\
`expert_fit`` ``<-`` `[`sort_validity`](https://juhalt.github.io/contentvalidR/reference/sort_validity.md)`(``sort_dat``, judge_type ``=`` ``"expert"``)`\
...
\
[`plot`](https://rdrr.io/r/graphics/plot.default.html)`(``fit``, metric ``=`` ``"psa"``)`

![](item-sort-validity_files/figure-html/unnamed-chunk-8-1.png)

\
[`plot`](https://rdrr.io/r/graphics/plot.default.html)`(``fit``, metric ``=`` ``"csv"``)`
...
\
`plan`` ``<-`` `[`sort_power`](https://juhalt.github.io/contentvalidR/reference/sort_power.md)`(``N ``=`` `[`seq`](https://rdrr.io/r/base/seq.html)`(``10``, ``50``, by ``=`` ``5``)``, true_p ``=`` `[`c`](https://rdrr.io/r/base/c.html)`(``.60``, ``.70``, ``.80``)``)`\
[`plot`](https://rdrr.io/r/graphics/plot.default.html)`(``plan``)`

![](item-sort-validity_files/figure-html/unnamed-chunk-10-1.png)

\
[`plot`](https://rdrr.io/r/graphics/plot.default.html)`(``plan``, type ``=`` ``"critical"``)`

...
\
[`attr`](https://rdrr.io/r/base/attr.html)`(`[`contentvalid_glossary`](https://juhalt.github.io/contentvalidR/reference/contentvalid_glossary.md)`(``)``, ``"statuses"``)`\
`#>              status`\
`#> 1         Supported`\
`#> 2            Review`\
`#> 3 Insufficient data`\
...
\
`sorts`` ``<-`` `[`read.csv`](https://rdrr.io/r/utils/read.table.html)`(`\
`  `[`system.file`](https://rdrr.io/r/base/system.file.html)`(``"extdata"``, ``"sort_example.csv"``, package ``=`` ``"contentvalidR"``)``,`\
`  stringsAsFactors ``=`` ``FALSE`\
`)`\
`fit_sort`` ``<-`` `[`sort_validity`](https://juhalt.github.io/contentvalidR/reference/sort_validity.md)`(``sorts``)`\
`fit_sort`\
...
\
`ratings`` ``<-`` `[`read.csv`](https://rdrr.io/r/utils/read.table.html)`(`\
`  `[`system.file`](https://rdrr.io/r/base/system.file.html)`(``"extdata"``, ``"rating_example.csv"``, package ``=`` ``"contentvalidR"``)``,`\
`  stringsAsFactors ``=`` ``FALSE`\
`)`\
`fit_rating`` ``<-`` `[`rating_validity`](https://juhalt.github.io/contentvalidR/reference/rating_validity.md)`(``ratings``)`\
`fit_rating``$``scale_summary``[``, `[`c`](https://rdrr.io/r/base/c.html)`(``"target"``, ``"mean_htc"``, ``"htc_strength"``,`\
...
\
`relevance`` ``<-`` `[`read.csv`](https://rdrr.io/r/utils/read.table.html)`(`\
`  `[`system.file`](https://rdrr.io/r/base/system.file.html)`(``"extdata"``, ``"expert_relevance_example.csv"``, package ``=`` ``"contentvalidR"``)``,`\
`  stringsAsFactors ``=`` ``FALSE`\
`)`\
`panel`` ``<-`` `[`as.matrix`](https://rdrr.io/r/base/matrix.html)`(``relevance``[``, `[`setdiff`](https://rdrr.io/r/base/sets.html)`(`[`names`](https://rdrr.io/r/base/names.html)`(``relevance``)``, ``"expert"``)``]``)`\
`fit_expert`` ``<-`` `[`expert_validity`](https://juhalt.github.io/contentvalidR/reference/expert_validity.md)`(``panel``, mode ``=`` ``"relevance"``, lo ``=`` ``1``, hi ``=`` ``4``)`\
`fit_expert``$``results``[``, `[`c`](https://rdrr.io/r/base/c.html)`(``"item"``, ``"N"``, ``"V"``, ``"I_CVI"``, ``"kappa_mod"``, ``"recommendation"``)``]`\
...
`fit_judge`` ``<-`` `[`judge_validity`](https://juhalt.github.io/contentvalidR/reference/judge_validity.md)`(``judge_ratings``, lo ``=`` ``1``, hi ``=`` ``4``)`\
`fit_judge``$``results``[``, `[`c`](https://rdrr.io/r/base/c.html)`(``"judge"``, ``"mean_rating"``, ``"severity_raw"``,`\
...
\
[`summary`](https://rdrr.io/r/base/summary.html)`(``fit_judge``)``$``gtheory``$``judges_needed`\
...
`  stringsAsFactors ``=`` ``FALSE`\
`)`\
`fit_domain`` ``<-`` `[`domain_validity`](https://juhalt.github.io/contentvalidR/reference/domain_validity.md)`(`\
`  ``assignments``,`\
`  cell_col ``=`` ``"construct"``,`\
`  domain ``=`` `[`c`](https://rdrr.io/r/base/c.html)`(``"Autonomy"``, ``"Competence"``, ``"Relatedness"``, ``"Belonging"``)`\
`)`\
`fit_domain``$``results``[``, `[`c`](https://rdrr.io/r/base/c.html)`(``"cell"``, ``"n_items"``, ``"share"``, ``"recommendation"``)``]`\
...
\
`fit_sort``$``settings``[`[`c`](https://rdrr.io/r/base/c.html)`(``"p0"``, ``"alpha"``, ``"judge_type"``)``]`\
`#> $p0`\
`#> [1] 0.5`\
`#> `\
`#> $alpha`\
...
\
[`options`](https://rdrr.io/r/base/options.html)`(``contentvalidR.show_key ``=`` ``FALSE``)`

The definitions remain available at any time:

\
[`contentvalid_glossary`](https://juhalt.github.io/contentvalidR/reference/contentvalid_glossary.md)`(``"expert-panel"``)`

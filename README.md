...
fitted `sort_validity()`, `rating_validity()`, or `expert_validity()`
...
`summary()` uses the same common count fields across all three
workflows, and `print()`/`plot()` retain method-appropriate displays.
Compatibility aliases such as `rating_fit$contrasts` and
...
`sort_validity()`, `rating_validity()`, and `expert_validity()`. Their
existing low-level function names are retained through the first public
